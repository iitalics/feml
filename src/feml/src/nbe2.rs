use crate::domain2::{self as dmn, Env, Idx, Lvl, Term, Val};
...
pub fn eval(gc: &mut Gc, stash: &gc::RootSet) {
    ...
    match tm.into() {
        Term::Con(t) => {
            stash.save(dmn::con_val(gc, t.con()));
        }
        Term::Var(t) => {
            stash.save(env);
        ...
        Term::Fn(t) => {
            stash.save(t.body());
            stash.save(env);
            stash.save(dmn::fn_val(gc, t.var(), stash));
        }
        Term::Pi(t) => {
            let var = t.var();
            stash.save(t.rng());
            ...
            stash.save(rng);
            stash.save(env);
            stash.save(dmn::pi_val(gc, var, stash));
        }
    }
}
...
pub fn apply<'gc>(gc: &'gc mut Gc, stash: &gc::RootSet) {
    ...
    match fun.into() {
        ...
        Val::Con(_) | Val::Neu(_) | Val::App(_) => {
            stash.save(fun);
            stash.save(arg);
            stash.save(dmn::app_val(gc, &stash));
        }
...
pub fn close<'gc>(gc: &'gc mut Gc, stash: &gc::RootSet) {
    ...
    stash.save(dmn::ext_env(gc, stash)); // :: body env
    eval(gc, stash)
}
...
pub fn reify<'gc>(gc: &'gc mut Gc, ctx: Lvl, stash: &gc::RootSet) {
    ...
    match val.into() {
        Val::Con(v) => {
            stash.save(dmn::con_term(gc, v.con()));
            //for arg in v.args() {...}
        }
        Val::Neu(v) => {
            let lvl = v.lvl();
            assert!(lvl <= ctx);
            let idx = ctx - lvl;
            stash.save(dmn::var_term(gc, idx));
            //for elim in v.elims() {...}
        }
        Val::App(v) => {
            stash.save(v.arg());
            ...
            stash.swap();
            reify(gc, ctx, stash);
            stash.save(dmn::app_term(gc, stash));
        }
        Val::Fn(v) => {
            let var = v.var();
            stash.save(v.body());
            stash.save(v.env());
            stash.save(dmn::neu_val(gc, ctx + 1));
            close(gc, stash);
            reify(gc, ctx + 1, stash);
            stash.save(dmn::fn_term(gc, var, stash));
        }
        Val::Pi(v) => {
            let var = v.var();
            stash.save(v.env());
            ...
            stash.save(rng);
            stash.save(env);
            stash.save(dmn::neu_val(gc, ctx + 1)); // :: dom rng env arg
            close(gc, stash); // :: dom rng
            reify(gc, ctx + 1, stash); // :: dom rng
            stash.save(dmn::pi_term(gc, var, stash));
        }
    }
}
