use crate::domain2 as dmn;
use crate::domain2::{Lvl, Term, Val};
use crate::gc::{self, Gc, Hndl};
use crate::intern::{self, Symbol};
use crate::nbe2 as nbe;
...
impl Context {
    ...
    pub fn env(&self, gc: &mut Gc) {
        self.stash.save(dmn::neu_env(gc, self.level()));
    }
...
fn syn_app(
    ...
    cx.stash.save(fun_tm);
    cx.stash.save(arg_tm);
    cx.stash.save(dmn::app_term(gc, &cx.stash));
...
fn syn_lambda(gc: &mut Gc, cx: &mut Context, lambda: &ast::Lambda<'_, '_>) -> Result<()> {
    ...
    cx.env(gc);
    cx.stash.save(dmn::pi_val(gc, Some(param_sym), &cx.stash));
    // :: body_tm lambda_ty

    cx.stash.swap();
    cx.stash.save(dmn::fn_term(gc, Some(param_sym), &cx.stash));
    cx.stash.swap();
    Ok(())
}
...
fn chk_lambda(gc: &mut Gc, cx: &mut Context, lambda: &ast::Lambda<'_, '_>) -> Result<()> {
    ...
    cx.stash.save(goal_rng_ty_tm);
    cx.stash.save(goal_rng_env);
    cx.stash.save(dmn::neu_val(gc, cx.level()));
    nbe::close(gc, &cx.stash);
...
fn syn_arrow(gc: &mut Gc, cx: &mut Context, arrow: &ast::Arrow<'_, '_>) -> Result<()> {
    ...
    // ⊢ (x:t) -> s ⇒ Π(x:t').s' : TYPE
    cx.stash.save(dmn::pi_term(gc, param_sym, &cx.stash));
    cx.stash.save(dmn::type_type());
    Ok(())
}
